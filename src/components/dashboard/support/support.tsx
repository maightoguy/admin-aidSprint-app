...
import { supportTicketSeeds, SupportTicket, SupportTicketStatus } from "./support.data";
...
export default function SupportPage() {
  ...
  const handleUpdateStatus = (ticketId: string, status: SupportTicketStatus) => {
    setTickets((prev) =>
      prev.map((t) => (t.id === ticketId ? { ...t, status, updatedAt: new Date().toLocaleDateString() } : t))
    );
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={cn(
                            ...
                            ticket.status === "Open" && "bg-[#E8F8EE] text-[#22C55E]",
                            ticket.status === "Pending" && "bg-[#FEF3E8] text-[#F79009]",
                            ticket.status === "Resolved" && "bg-[#FEE4E2] text-[#F04438]"
                          )}
                        >
                          {ticket.status}
                        </span>
